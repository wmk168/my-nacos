...
/**
 * sentinel 流控集群测试通过
 * 1、参考案例：https://gitee.com/all_4_you/sentinel-tutorial/blob/master/sentinel-practice/sentinel-cluster-flow-control/sentinel-cluster-flow-environment-build.md
 * 2、创建MyClusterServer类
 * 3、initClusterFlowSupplier 必须要创建,客户端创建不创建无所谓
 * 4、控制台绑定server port端口最好不要是tokenServer服务端口
 * 5、客户端只需要引入包即可
 * 6、一个新问题：tokenServer又无法集群，如果挂了，集群基本就失效了
 **/
public class MyClusterServer {

    private static final String APP_NAME = "my-sentinel" //"appA"
    		;
    ...
    public static void main(String[] args) throws Exception {
    	...
    	System.setProperty("project.name", APP_NAME);
    	System.setProperty("csp.sentinel.log.use.pid", "true");
    	System.setProperty("csp.sentinel.dashboard.server", "http://127.0.0.1:8080");
    	
        MyClusterServer clusterServer = new MyClusterServer();
        clusterServer.init();
        clusterServer.start();
    }
